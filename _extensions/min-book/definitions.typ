...
//
// MIN-BOOK FIX: Explicitly calculate side parameter to work around an issue
// where marginalia's internal here().page() returns incorrect values when
// nested inside min-book's context block. We calculate the side in our context
// where here().page() is correct, then pass it explicitly to note().
#let column-sidenote(body) = {
  context {
    let num = counter(footnote).display("1")
    // Calculate side explicitly for book mode recto/verso placement
    // On even (verso) pages, outer margin is on the left
    // On odd (recto) pages, outer margin is on the right
    let page = here().page()
    let side = if calc.even(page) { "left" } else { "right" }
    ...
    note(
      ...
      side: side,     // Explicit side to bypass marginalia's page detection
    )[
      #super(num) #body
    ]
  }
